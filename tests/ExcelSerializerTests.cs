...
public class ExcelSerializerTests
{
    ...
    [Fact]
    public void ExportThenImport_WithStream_ShouldReturnEquivalentObjects()
    {
        //Arrange
        var serializer = new ExcelSerializer();
        var originalList = new List<ExampleModel> {
        ...
        //Act
        serializer.ExportToExcel(originalList, memoryStream);
        memoryStream.Position = 0; //Reset for reading

        var importedList = serializer.ImportFromExcel<ExampleModel>(memoryStream);
